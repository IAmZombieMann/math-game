...
const strategyHighlight = document.getElementById("strategy-highlight");
const counterDisplay = document.getElementById("counter-display");
const counterButtons = document.querySelectorAll(".counter-btn[data-step]");
...
const strategyHighlights = {
  count2: "💖 Usa parejas: 2, 4, 6, 8...",
  count3: "💙 Salta en tríos: 3, 6, 9, 12...",
  count4: "💜 Piensa en dobles de 2 para llegar de 4 en 4.",
  placeValue: "💛 345 = 3 centenas, 4 decenas, 5 unidades."
};
...
function renderChallenge(challenge) {
  ...
  strategyHighlight.textContent = strategyHighlights[challenge.type];
  helperText.textContent = challenge.helper;
  updateCounterTip(challenge.type);
...
function updateStats() {
  animateCounter(scoreEl, gameState.score);
  animateCounter(streakEl, gameState.streak);
  animateCounter(levelEl, gameState.level);
  animateCounter(timerEl, gameState.timeLeft);
}
...
function animateCounter(element, nextValue) {
  ...
  const step = nextValue > currentValue ? 1 : -1;
  const diff = Math.abs(nextValue - currentValue);
  const increment = diff > 20 ? step * 2 : step;
  element.textContent = currentValue + increment;
  requestAnimationFrame(() => animateCounter(element, nextValue));
}
...
function changeCounter(step) {
  counterValue = Math.max(0, counterValue + step);
  updateCounterDisplay();
}

function updateCounterTip(type) {
  if (type === "count2") {
    counterDisplay.style.boxShadow = "inset 0 0 0 2px rgba(125, 220, 255, 0.5)";
  } else if (type === "count3") {
    counterDisplay.style.boxShadow = "inset 0 0 0 2px rgba(123, 242, 211, 0.5)";
  } else if (type === "count4") {
    counterDisplay.style.boxShadow = "inset 0 0 0 2px rgba(169, 137, 255, 0.5)";
  } else {
    counterDisplay.style.boxShadow = "inset 0 0 0 2px rgba(255, 143, 199, 0.5)";
  }
}
...
function startCheerRotation() {
  ...
  cheerInterval = setInterval(() => {
    index = (index + 1) % cheerMessages.length;
    cheerText.textContent = cheerMessages[index];
  }, 2300);
}
...
counterButtons.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    changeCounter(step);
    playTone(540 + step * 80, 0.08, "triangle", 0.035);
  });
});
